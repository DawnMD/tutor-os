...
export const OwnerSessionCalendar = ({ batchId }: { batchId: string }) => {
  const { data: batchSchedules } = useQuery(
    orpc.owner.batch.getBatchSchedulesById.queryOptions({
      input: {
        batchId: batchId,
      },
    }),
  ...
  return (
    <section className="flex w-full h-full items-start justify-center bg-muted/30 text-foreground">
              ...
              {monthCells.map((day, idx) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "group flex min-h-20 min-w-0 flex-col gap-1 border-border p-1.5 text-left align-top transition-colors sm:min-h-24",
                      idx % 7 !== 6 && "border-r",
                      !isLastRow && "border-b",
              ...
              {weekDays.map((d, idx) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex min-h-56 min-w-0 flex-col gap-2 border-border p-2 text-left transition-colors",
                      idx !== 6 && "border-r",
                      "hover:bg-accent/40",
                      isSelected && "bg-accent/60",
                    )}
