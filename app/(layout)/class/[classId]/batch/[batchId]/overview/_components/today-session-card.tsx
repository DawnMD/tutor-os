...
export default function TodaySessionCard({ batch }: TodaySessionCardProps) {
  ...
  const todaySession = batch.sessions.find((session) => {
  ...
  const todaySchedule = batch.schedules.find(
    (schedule) => schedule.dayOfWeek === today.getDay(),
  );
  ...
  return (
    <Card className="border-l-4 border-l-primary bg-linear-to-br from-primary/5 to-transparent">
      <CardHeader>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
          <div>
            ...
            <p className="font-semibold">
              {formatTime(todaySchedule.startMinutes)} -{" "}
              {formatTime(todaySchedule.endMinutes)}
            </p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground mb-1">Topic</p>
            <p className="font-semibold text-sm">
              {todaySession?.topic || "Not set"}
            </p>
          </div>
          {todaySession && (
            <div>
