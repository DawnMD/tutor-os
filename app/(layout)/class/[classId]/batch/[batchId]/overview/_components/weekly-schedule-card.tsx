...
export default function WeeklyScheduleCard({ schedules }: WeeklyScheduleCardProps) {
          ...
          {sortedSchedules.map((schedule: any) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-sm">{daysOfWeek[schedule.dayOfWeek]}</p>
                  <p className="text-xs text-muted-foreground">
                    {formatTime(schedule.startMinutes)} – {formatTime(schedule.endMinutes)}
                  </p>
                </div>
              </div>
